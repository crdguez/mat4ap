...
\newcommand{\class}{Mat. Aplicadas 4º Agrupado}
...
\question [1] Raquel, María e Isabel han ganado un premio de 8000\euro en un sorteo. Sabiendo que, para comprar los boletos, Raquel puso 5\euro, María 8\euro e Isabel 12\euro, ¿cuánto le corresponderá a cada una del premio que han ganado?
\begin{solution}
1600, 2560, 3840\euro
\end{solution}
...
$x=\frac{3}{4}$
\end{solution}
\part[2] $x+\dfrac{3(x-2)}{9}=\dfrac{5(x-1)}{4}+\dfrac{7}{12}$
\begin{solution}
$x=0$
\end{solution}
\part[1] $x^2-2x-8=0$  
...
\question[2] Cuatro barras de pan y seis litros de leche cuestan 6,80; tres barras de pan y cuatro
...
%\draw[color=black] (0pt,-10pt) node[right] {\footnotesize $0$};
%\clip(-0.6129302567150502,-0.43158220601634095) rectangle (9.010648940148005,7.8783927087822985);
\draw[->, color=red, domain=0  :16 + 0.1]    plot (\x,{1/4*(\x) + 3.5}) node[right] {};
\end{tikzpicture}
...
\begin{solution} $y=0.25x+3.5$ e $y=0.25\cdot30 +3.5=11$ \euro
\end{solution}
%\part[1] Indica el dominio y el recorrido de la función
%\begin{solution} $(0,+\infty)$ y $(\frac{7}{2},+\infty)$. Se desprecia que el número de horas de un mes es finito 
%\end{solution}
\end{parts}
